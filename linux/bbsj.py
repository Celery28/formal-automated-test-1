...
class bbs(object):
    ...
    def openC(self,url,Account,Password):
        ...
        try:
            ...
            driver.save_screenshot("./report/screen_shot/failed/test_%s.png" % (bbs().get_current_function_name()))
            return 'false'
    def sign(self,a):
        '''
        签到
        ...
        try:
            ...
            if '%s'%Front <= '%s'%Back:
                ...
                driver.save_screenshot("./report/screen_shot/failed/test_%s.png" %(bbs().get_current_function_name()))
                return 'false签到前%s后%s'%(Front,Back)
        except:
            driver.save_screenshot("./report/screen_shot/failed/test_%s.png" % (bbs().get_current_function_name()))
            return '代码有错false'
    # def Selected(self):
    ...
    def post(self):
        ...
        '''
        try:
            driver.find_element_by_xpath('/html/body/div[2]/div[2]/div[3]/h2/span[7]').click()
            # a楼层总数
            a = driver.find_elements_by_xpath('//*[@id="treeNav"]/li')
            b = random.randint(1, len(a))
            print(b)
...
if __name__=='__main__':
    ...
    # bbs().Selected()
    bbs().openC('http://www.kgc.cn/bbs','13581917640','910227')
    print(bbs().follow())
