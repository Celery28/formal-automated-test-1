...
class PersonalC(object):
    ...
    def openC(self,url,Account,Password):
        ...
        try:
            ...
            driver.save_screenshot("test_%s.png" % (PersonalC().get_current_function_name()))
            s = sys.exc_info()
            return '代码有错%s' % (s[1])
    ...
    def mycourse(self):
        ...
        try:
            ...
            if '%s' % name2 == '%s' % name1 and '%s'%name == '%s'%name3:
                return 'true'
            else:
                driver.save_screenshot("test_%s.png" % (PersonalC().get_current_function_name()))
                return 'false'
            ...
            if len(count)>1:
                ...
                if '%s'%name=='%s'%name1:
                    try:
                        ...
                        driver.find_element_by_xpath('/html/body/div[4]/div/div/div[2]/div[4]/div[3]/div/a').click()
                    driver.switch_to_window(driver.window_handles[0])
